...
Label(sroot,text="Welcome to My Hotel Management App",font='Timesnewroman 40 ',bg='white',fg='black').place(x=100,y=210)
...
def mainroot():
	...
	# root.title("Made with love by Sadat")
	#--------------seperator-------------------------------------------------------------------------------------------------------------------

	sep = Frame(height=500,bd=1,relief='sunken',bg='white')
	#sep.place(x=20,y=0)
	#----------------Connection with printer To Print Receipts---------------------------------------------------------------------------------------------
	...
	def hotel_status():
		...
		x = HotelStatus.checkStatus()
		#print(x)

		x = HotelStatus.countRoom()
		print (x)

		y = HotelStatus.isReserved()
		print (y)
		tor = x[0]
		rer = y[0]
		tos = 21
	...
	def staff():
		...
		b_frame.pack_propagate(False)
		b_frame.tkraise()
		nl = Label(b_frame,text='Made by Sadat',fg='black',bg='gray91',font='msserif 8')
		nl.place(x=955,y=310)
		nl.tkraise()
	...
	def rooms():
		...
		def roomdet(rno):
			...
			rdata=RoomDetails.roomDetail(rno)
			#print (rdata)
	...
	#cur.execute("create table if not exists paymentsf(id number  primary key,f_name varchar,l_name varchar,c_number varchar,email varchar , r_n number ,day varchar,month varchar,year varchar,time varchar , method varchar,totalamt varchar)")
			...
			x = PaymentData.checkPayment(idd)

			#print(x)
			yy = PaymentData.duration(idd)
			print (yy)
			#print (x)
			if(yy!=None):
				dot = yy[0]+'-'+yy[1]+'-'+yy[2]
	...
	#cur.execute("create table if not exists paymentsf(id number  primary key,f_name varchar,l_name varchar,c_number varchar,email varchar , r_n number ,day varchar,month varchar,year varchar,time varchar , method varchar)")
				...
				temp = BookingData.roomStatus(roomn)
				if temp[0] == 'Reserved':
					...
					rp = BookingData.sendPrice(roomn)
					print (rp)
		...
		def findrooms():
			...
			x = RoomDetails.findrooms(nb.get(), ac.get(), tv.get(), wifi)
			#print (x)
